...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	{
		LogTool.Log ("ResizeTexture " + m_iWidth + " " + m_iHeight);
    ...
    {
        LogTool.Log("ApplicationPause : " + bPause);
        if (bPause == true)
        {
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
        ...
        loader = new Thread ( () => {
			...
			if (ffmpeg.avformat_open_input(&ppFomatContext, strFileName, null, null) != 0)
			{
                ...
                LogTool.Log("Could not open file");
			...
			{
				m_CurrentState = MEDIAPLAYER_STATE.ERROR;
                LogTool.Log("Could not find stream info");
	...
	void LoadVideoPart2 ()
	{
		...
		for (var i = 0; i < (pFormatContext)->nb_streams; i++)
		{
			if ((pFormatContext)->streams[i]->codec->codec_type == AVMediaType.AVMEDIA_TYPE_VIDEO)
			{
				if(bFindVideo == false)
				{
					...
					LogTool.Log("Video" +  iStreamIndex);
				}
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            LogTool.Log("Could not found video stream");
		...
		{
			//m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            LogTool.Log("Could not found Audio stream" + bFindAudio);
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            LogTool.Log("Could not initialize the conversion context");
		...
		{
			m_CurrentState = MEDIAPLAYER_STATE.ERROR;
            LogTool.Log("Could not open codec");
	...
	{
		LogTool.Log("EasyMovieTexture: " + message);
	}
			...
			if (pPacket != null) 
			{
				...
				if (pPacket->stream_index == iStreamIndex) {
					...
					if (size < 0) {
                        ...
                        LogTool.Log("Error while decoding frame");
                        return;
						...
						if (pts > 0) {
							if (listVideo.Count > 5)
							{
								if (!m_bIsFirstFrameReady) {
									...
									//LogTool.Log("FistReady"  + listVideo.Count);
									m_bIsFirstFrameReady = true;
			...
			do{
				...
				if ( ret < 0)
				{
					...
					{   
                        ...
                        LogTool.Log("Could not read frame");
                    }
				...
				// LogTool.Log(pPacket->pts + " " + pCodecContext->pts_correction_last_pts );
						...
						if (iAudioLen >= 0) {
							if (gotSound == 1) {
								...
								//if( pAudioCodecContext->sample_fmt != AVSampleFormat.AV_SAMPLE_FMT_FLT)
								{
									//for(int i = 0; i < pAudioCodecContext->channels; i++)
									{
										...
										if( bSeekTo == true)
										{
											...
											//LogTool.Log(value + " " + pts + " " + GetDuration() + " " + pDecodedAudioFrame->pkt_duration);
										...
										//LogTool.Log ("sound " + iSoundCount);
										//LogTool.Log (pDecodedAudioFrame->pkt_dts + " " +pDecodedAudioFrame->pkt_duration + " "  + pDecodedAudioFrame->pkt_pos + " " + pDecodedAudioFrame->pkt_pts + " " + pts);
										...
										lock( listAudio)
										{
											...
											{ 
												...
												{
													//LogTool.Log(pDecodedAudioFrame->pkt_pts % pDecodedAudioFrame->pkt_duration + " " + iDataSize2);
										...
										//LogTool.Log ( iDataSize + " "+ pts + " " + pAudioCodecContext->sample_rate + " " + pDecodedAudioFrame->sample_rate + " "+ pDecodedAudioFrame->nb_samples + " " + pDecodedAudioFrame->pkt_pts + " " + pDecodedAudioFrame->pkt_pos);
										//LogTool.Log ("sound decode time " + pts);
								...
								/*else
								{
									...
									//LogTool.Log ( pDecodedAudioFrame->channels + " "+buffer.Length / 4 + " " + pDecodedAudioFrame->pkt_pts + " " + pDecodedAudioFrame->pkt_pos);
									//LogTool.Log ("sound decode time " + pts);
	...
	{
	//LogTool.Log (newPosition);

	//LogTool.Log("Pts" +  listAudioPtsTime[0]);
	}*/

	void OnAudioRead(float[] data)
	{
		//LogTool.Log (listAudio.Count);

		if (listAudio.Count < 3)
		{
			return;
		}
		//LogTool.Log (data.Length + " " + listAudio[1].Length);
		...
		if (iSoundBufferCount < data.Length) {
			...
			//LogTool.Log (iSoundBufferCount + " " + data.Length + " " + listAudio [1].Length);
	...
	private void Call_UpdateVideoTexture()
	{
		...
		//LogTool.Log("last " + fLastFrameTime + " " + fCurrentSeekTime + " " + listVideo.Count);
					...
					if( listAudioPts.Count > i)
					{
						if (listAudioPts [i] >= 0) {

							{
								...
								{
									//LogTool.Log(audioSource.time  + " " + (float)listAudioPtsTime[i]  + " " +listAudioPts [i]   +" " +  fLastFrameTime);
		...
		else
		{
			...
			//do
			{
				...
				if( audioSource != null &&  Call_GetDuration() > 0)
				{
					...
					{
						//LogTool.Log("sync1 " + audioSource.time + " " + fLastFrameTime);
						audioSource.time = fLastFrameTime%600.0f;
					}
	...
	private void Call_SetSeekPosition(int iSeek)
	{
		...
		//LogTool.Log (fLastFrameTime + " " + fCurrentSeekTime);
	...
	private int Call_GetVideoWidth()
	{
		//LogTool.Log (m_iWidth);
		return m_iWidth;
	}

	private int Call_GetVideoHeight()
	{
		//LogTool.Log (m_iHeight);
		return m_iHeight;
	}
	...
	private int Call_GetDuration()
	{
		//LogTool.Log ((int)(pFormatContext->duration / 1000));
	...
	public IEnumerator DownloadStreamingVideoAndLoad(string strURL)
	{
		...
		LogTool.Log("DownloadStreamingVideo : " + strURL);
	...
	IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
	{
		...
		{
			LogTool.Log("CopyStreamingAssetVideoAndLoad : " + strURL);
